...
fn semantic_metadata_for(command: &str) -> Option<Value> {
    let metadata = match command {
        ...
        "queries schema" => json!({
            "domain": "queries",
            "description": "Lista datasets, dimensiones, medidas, filtros y campos ordenables del contrato QuerySpec seguro, incluyendo abastecimiento cuando la API lo expone.",
            "tags": ["queries", "queryspec", "schema", "analytics", "inventory", "purchases", "procurement"],
            "examples": ["datasets disponibles", "campos para queryspec", "schema queries", "datasets de inventario", "campos de compras"],
            "capabilities": {
                "entity": "query_dataset",
                "grain": "dataset",
                "measures": [],
                "dimensions": ["name", "label", "description", "required_scope", "dimensions", "measures", "filters", "sortable_fields", "limitations"],
                "supported_operations": ["discover", "list"],
                "default_rank": ["name"],
                "active_condition": [],
                "supports_period": false,
                "semantic_aliases": {
                    "datasets": ["datasets", "tablas disponibles", "fuentes disponibles", "fuentes de abastecimiento"],
                    "measures": ["metricas", "medidas", "valores"],
                    "dimensions": ["dimensiones", "campos", "agrupaciones"],
                    "filters": ["filtros"],
                    "sortable_fields": ["ordenar", "sort", "ranking"],
                    "inventory_stock": ["inventario", "stock", "bajo stock", "existencias"],
                    "inventory_movements": ["movimientos", "entradas", "salidas", "consumo de insumos"],
                    "purchase_items": ["compras", "proveedores", "abastecimiento", "insumos comprados"]
                },
                "answer_patterns": ["descubrir queryspec", "listar datasets", "ver campos disponibles", "descubrir inventario", "descubrir compras"],
                "join_keys": ["name"],
        ...
        "queries run" => json!({
            "domain": "queries",
            "description": "Ejecuta un QuerySpec seguro validado por la API y devuelve filas analiticas normalizadas para ventas, clientes, rentabilidad, inventario, movimientos y compras.",
            "tags": ["queries", "queryspec", "analytics", "dynamic", "inventory", "purchases", "procurement"],
            "examples": ["productos mas vendidos con margen", "clientes por ticket promedio", "productos con utilidad baja", "insumos en bajo stock", "movimientos de inventario por dia", "compras por proveedor"],
            "capabilities": {
                "entity": "query_row",
                "grain": "dynamic_dataset_row",
                "measures": ["quantity_sold", "revenue", "orders_count", "avg_price", "order_count", "total_spent", "avg_ticket", "waros_balance", "profit_per_unit", "profit_margin_pct", "profit_margin_real_pct", "profit_margin_operativo_pct", "total_profit", "current_stock", "minimum_stock", "maximum_stock", "unit_cost", "stock_value", "quantity_in", "quantity_out", "net_quantity", "movement_count", "avg_cost_per_unit", "quantity_purchased", "quantity_received", "total_cost", "avg_unit_cost", "purchase_count"],
                "dimensions": ["product", "product_id", "category", "day", "customer", "customer_id", "classification", "cost_source", "ingredient", "ingredient_id", "unit", "status", "movement_type", "reference_table", "reference_number", "supplier", "supplier_id", "purchase_number", "is_direct_entry"],
                "supported_operations": ["filter", "aggregate", "group", "rank", "sort", "limit", "compare"],
                "default_rank": ["revenue", "quantity_sold", "total_profit", "total_spent", "current_stock", "stock_value", "total_cost", "avg_unit_cost"],
                ...
                "semantic_aliases": {
                    ...
                    "ingredient": ["ingrediente", "insumo", "materia prima"],
                    "current_stock": ["stock actual", "existencias", "inventario"],
                    "minimum_stock": ["stock minimo", "punto de reposicion", "bajo stock"],
                    "stock_value": ["valor de inventario", "valor del stock"],
                    "quantity_in": ["entradas", "ingresos de inventario"],
                    "quantity_out": ["salidas", "consumo", "descuento de inventario"],
                    "net_quantity": ["movimiento neto", "balance de movimiento"],
                    "supplier": ["proveedor", "proveedores"],
                    "total_cost": ["costo total", "valor comprado", "gasto en compras"],
                    "avg_unit_cost": ["costo unitario promedio", "precio promedio de compra"]
                },
                "answer_patterns": ["analisis dinamico", "ranking con metricas", "comparar dimensiones", "diagnostico queryspec", "analisis de inventario", "analisis de compras", "ranking de proveedores"],
                "join_keys": ["product_id", "customer_id", "ingredient_id", "supplier_id"],
